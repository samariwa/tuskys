...
class ForgotPasswordController extends Controller
{
    ...
    public function sendResetLinkEmail(Request $request)
    {
        ...
        $sentinelUser = User::whereId($user->id);
        if (count($user) == 0) {
            return redirect()->back()->with('error_message', 'Kindly Confirm that the entered email was used in registration');
        }
        $reminder = Password_reset::exists($sentinelUser) ?: Password_reset::create($sentinelUser);
        $this->sendEmial($user, $reminder->token);
        return redirect()->back()->with('success_message', 'Reset link has been Sent to Your Email');
    }
